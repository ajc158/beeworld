...
/*!
 * \brief spineMLNetworkServer::recvName
 * \param ok
 * \return
 *
 * Get an int with the connection name length, then receive the name itself...
 */
bool spineMLNetworkServer::recvName() {
    ...
    n = connection->read((char *) &(size),sizeof(size));
    if (n != sizeof(size)) {
        qDebug() << "Error reading in name size";
        disconnectServer();
        return false;
    }

    qDebug() << "name size obtained (" << QString::number(size) << ")";
    ...
    // now use this value to obtain the string with the connection name
    //name.resize(size);
    char buffer[1000];
    n = connection->read(buffer,size);
    if (n < 0) {
        qDebug() << "Error reading in name";
        disconnectServer();
        return false;
    }
    ...
    n = connection->write(&sendVal,1);
    if (n < 1) {
        qDebug() << "Error writing in receive name";
        disconnectServer();
        return false;
    }
...
bool spineMLNetworkServer::recvData(char * data, int size) {
    ...
    // copy across data to a QVector and emit it
    QVector < float > datav;
    datav.resize(size);
    for (int i = 0; i < size; ++i) {
        datav[i] = ((double *) data)[i];
    }
    emit dataReceived(datav);
